# Managed Compute Deployments — Get Logs
...
POST https://management.azure.com/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}
    ...
    /managedComputeDeployments/{deploymentName}/getLogs?api-version=2026-03-15-preview
...
- **Authorization**: Requires `Microsoft.CognitiveServices/accounts/managedComputeDeployments/read` permission.
...
logs = cog.managed_compute_deployments.get_logs(
...
```bash
# Get inference server logs from all instances (default)
az cognitiveservices account managed-compute-deployment get-logs \
  --name $ACCOUNT -g $RG \
