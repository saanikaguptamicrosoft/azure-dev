# Managed Compute Deployments — Spec

## Overview

This spec defines the `managedComputeDeployments` resource type — a **new ARM resource** under `Microsoft.CognitiveServices/accounts` for managed compute (GPU-based) model deployments in Azure AI Foundry.
...
- Type-safe list operations: `managedComputeDeployments.list()` returns only managed compute deployments.
...
All three inputs (`model`, `deploymentTemplate`, `acceleratorType`) are **required** for creating an managed compute deployment.
...
|---|---|---|---|---|
| [**Create or Update**](deployments_crud/create-or-update.md) | `PUT` | `/{deploymentName}` | Yes (10–15 min) | Create a new deployment or update an existing one (change template). |
| [**Update (Scale)**](deployments_crud/patch.md) | `PATCH` | `/{deploymentName}` | Yes | Update SKU (scale instance count). Preferred over PUT for scaling. |
| [**Get**](deployments_crud/get.md) | `GET` | `/{deploymentName}` | No | Retrieve deployment properties, provisioning state, and inference routes. |
| [**List**](deployments_crud/list.md) | `GET` | `/` | No | List all managed compute deployments under the account. |
...
```json
{
  "id": "/subscriptions/{sub}/resourceGroups/{rg}/providers/Microsoft.CognitiveServices/accounts/{account}/managedComputeDeployments/{name}",
  "name": "{deploymentName}",
...
|---|---|---|---|
| `properties.model` | **Required** | Yes | ML Registry model asset URI (catalog models) or `ManagedComputeDeploymentModel` object (custom models). Immutable after creation. See [spec-custom-models.md](spec-custom-models.md) for custom model format. |
| `properties.deploymentTemplate` | **Required** | Yes | ML Registry deployment template URI. Can be updated (triggers rolling update). |
| `properties.acceleratorType` | **Required** | Yes | Accelerator type (e.g., `H100_80GB`). **Immutable after creation** — changing accelerator type requires deleting and recreating the deployment. |
...
| `properties.provisioningState` | — | **Read-only** | Current state: `Accepted`, `Creating`, `Updating`, `Succeeded`, `Failed`, `Deleting`, `Canceled`, `Moving`, `ResolvingDNS`. |
| `properties.provisioningDetails` | — | **Read-only** | Status message and timestamp. |
| `properties.routes` | — | **Read-only** | Inference route paths relative to the account endpoint root, including the full `/managedComputeDeployments/{deploymentName}/` prefix. Sourced from deployment template. Populated when `Succeeded`. |
...
poller = cog.managed_compute_deployments.begin_create_or_update(rg, account, name, deployment)
...
az cognitiveservices account managed-compute-deployment create ... --no-wait

# Check state later
az cognitiveservices account managed-compute-deployment show ... --query "properties.provisioningState"

# Block until a specific state
az cognitiveservices account managed-compute-deployment wait ... --created
```
...
|---|---|---|
| Create / Update | `Microsoft.CognitiveServices/accounts/managedComputeDeployments/write` | Create or update a managed compute deployment. |
| Get | `Microsoft.CognitiveServices/accounts/managedComputeDeployments/read` | Read deployment details. |
| List | `Microsoft.CognitiveServices/accounts/managedComputeDeployments/read` | List all managed compute deployments. |
| Delete | `Microsoft.CognitiveServices/accounts/managedComputeDeployments/delete` | Delete a managed compute deployment. |
...
| **Reader** | ❌ | ✅ | ❌ |
| **Cognitive Services Managed Compute Deployment Operator** *(proposed)* | ✅ | ✅ | ✅ |

The proposed **Cognitive Services Managed Compute Deployment Operator** role grants full CRUD access to managed compute deployments only. This enables separation of duties — teams managing GPU infrastructure can operate independently.

#### Custom Role Example

```json
{
  "Name": "Managed Compute Deployment Operator",
...
Azure Policy can be used to enforce governance rules on managed compute deployments. Policies evaluate the `PUT` request body and can `deny`, `audit`, or `modify` deployments.

### Policy Alias Paths

| Alias | Type | Description |
|---|---|---|
| `Microsoft.CognitiveServices/accounts/managedComputeDeployments/properties.model` | `string` | The model being deployed. Use to allowlist/denylist specific models. |
| `Microsoft.CognitiveServices/accounts/managedComputeDeployments/properties.acceleratorType` | `string` | The accelerator type. Use to restrict which GPU types are allowed. |
| `Microsoft.CognitiveServices/accounts/managedComputeDeployments/sku.name` | `string` | The SKU name (`GlobalManagedCompute`, `DataZoneManagedCompute`). Use to restrict deployment to data-zone-scoped SKUs only. |
| `Microsoft.CognitiveServices/accounts/managedComputeDeployments/sku.capacity` | `integer` | Number of model instances (not accelerator count). Use to set max instance limits. To cap total GPU consumption, factor in the model's `acceleratorsPerInstance`. |
| `Microsoft.CognitiveServices/accounts/managedComputeDeployments/properties.deploymentTemplate` | `string` | The deployment template. Use to enforce approved templates. |
...
```json
{
  "if": {
    "allOf": [
      {
        "field": "type",
        "equals": "Microsoft.CognitiveServices/accounts/managedComputeDeployments"
      },
      {
        "field": "Microsoft.CognitiveServices/accounts/managedComputeDeployments/properties.model",
        "notIn": [
          "azureml://registries/azureml-openai-oss/models/gpt-oss-120b/versions/4",
...
```json
{
  "if": {
    "allOf": [
      ...
      {
        "field": "Microsoft.CognitiveServices/accounts/managedComputeDeployments/properties.acceleratorType",
...
```json
{
  "if": {
    "allOf": [
      ...
      {
        "field": "Microsoft.CognitiveServices/accounts/managedComputeDeployments/sku.capacity",
        "greater": 4
      }
    ]
  },
...
```json
{
  "if": {
    "allOf": [
      ...
      {
        "field": "Microsoft.CognitiveServices/accounts/managedComputeDeployments/sku.name",
...
|---|---|---|---|
| Create initiated | `Microsoft.CognitiveServices/accounts/managedComputeDeployments/write` | `Started` | `PUT` request accepted. |
| Create succeeded | `Microsoft.CognitiveServices/accounts/managedComputeDeployments/write` | `Succeeded` | Deployment healthy and serving. |
| Create failed | `Microsoft.CognitiveServices/accounts/managedComputeDeployments/write` | `Failed` | Provisioning failed. |
| Scale initiated | `Microsoft.CognitiveServices/accounts/managedComputeDeployments/write` | `Started` | `PUT` with new `sku.capacity`. |
| Scale succeeded | `Microsoft.CognitiveServices/accounts/managedComputeDeployments/write` | `Succeeded` | Scale-out/in complete. |
| Delete initiated | `Microsoft.CognitiveServices/accounts/managedComputeDeployments/delete` | `Started` | `DELETE` request accepted. |
| Delete completed | `Microsoft.CognitiveServices/accounts/managedComputeDeployments/delete` | `Succeeded` | Resources released. |
...
```bash
# Route managed compute deployment logs to Log Analytics
...
```kusto
// All managed compute deployment operations in the last 7 days
AzureActivity
| where ResourceProvider == "MICROSOFT.COGNITIVESERVICES"
| where OperationNameValue has "managedComputeDeployments"
...
AzureActivity
| where ResourceProvider == "MICROSOFT.COGNITIVESERVICES"
| where OperationNameValue has "managedComputeDeployments/write"
...
| **New subscriptions** | Weekly/monthly | Subscriptions that created their first-ever managed compute deployment in the period. | New customer acquisition. |
| **First deployment time-to-value** | Per subscription | Duration from first ARM API call (any `managedComputeDeployments` operation) to first `provisioningState = Succeeded`. | Onboarding friction. |
...
| **ARM API error rate** | Daily | 5xx responses / total requests for all `managedComputeDeployments` operations. | ≤ 0.1% |
...
Deployment names must be **unique across both `deployments` and `managedComputeDeployments`** on the same account. This is because inference routing uses the deployment name in the URL path — collisions would create ambiguous routes.

| Rule | Constraint |
|---|---|
| Length | 1–64 characters |
| Characters | Alphanumeric, underscore, hyphen, period: `^[a-zA-Z0-9][a-zA-Z0-9._-]{0,63}$` |
| Uniqueness | Unique within the account, across both `deployments` and `managedComputeDeployments`. |
...
# 1. Create deployment
curl -X PUT \
  "https://management.azure.com/subscriptions/$SUB/resourceGroups/$RG/providers/Microsoft.CognitiveServices/accounts/$ACCOUNT/managedComputeDeployments/gpt-oss-120b-gpu?api-version=2026-03-15-preview" \
...
# 5. Scale to 2 instances (use PATCH for SKU-only updates)
curl -X PATCH \
  "https://management.azure.com/subscriptions/$SUB/resourceGroups/$RG/providers/Microsoft.CognitiveServices/accounts/$ACCOUNT/managedComputeDeployments/gpt-oss-120b-gpu?api-version=2026-03-15-preview" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
from azure.mgmt.cognitiveservices.models import (
    ManagedComputeDeployment, ManagedComputeDeploymentProperties, Sku,
)
from azure.identity import DefaultAzureCredential
import time
...
poller = cog.managed_compute_deployments.begin_create_or_update(rg, account, "gpt-oss-120b-gpu", deployment)
result = poller.result()
print(f"Routes: {result.properties.routes}")

# 2. List
for dep in cog.managed_compute_deployments.list(rg, account):
    print(f"{dep.name}: {dep.properties.accelerator_type} × {dep.sku.capacity}")

# 3. Scale
scale = {"sku": {"name": "GlobalManagedCompute", "capacity": 2}}
cog.managed_compute_deployments.begin_update(rg, account, "gpt-oss-120b-gpu", scale).result()

# 4. Delete
cog.managed_compute_deployments.begin_delete(rg, account, "gpt-oss-120b-gpu").result()
```
...
# 2. List
az cognitiveservices account managed-compute-deployment list --name $ACCOUNT -g $RG -o table
