...
This extends the existing accelerator deployment spec ([spec-deployments.md](spec-deployments.md)) to cover **custom models** — models that are not in the Microsoft Foundry Model Catalog but are architecturally compatible with a catalog model's deployment templates.
...
| **List Models** | `GET` | `/models` | List all registered models.|
| **List Model Versions** | `GET` | `/models/{modelName}/versions` | List all versions for a model by name. |
| **Get Model** | `GET` | `/models/{modelName}/versions/{version}` | Get model details by name and version. |
| **Delete Model** | `DELETE` | `/models/{modelName}/versions/{version}` | Delete a registered model version. Blocked if active deployments exist. |
...
```http
GET {account}.services.ai.azure.com/api/projects/{project}/models/{modelName}/versions?api-version=2025-06-01-preview
Authorization: Bearer {token}
```
...
> The CLI design spec does not include an `--all-versions` flag on `custom list`. To inspect a specific model's versions, use `custom show` which defaults to the latest version:

```bash
# Show latest version for a model
azd ai models custom show --name my-gpt-oss-120B

# Show specific version
azd ai models custom show --name my-gpt-oss-120B --version 1
```
...
```http
GET {account}.services.ai.azure.com/api/projects/{project}/models/{modelName}/versions/{version}?api-version=2025-06-01-preview
Authorization: Bearer {token}
```
...
```http
DELETE {account}.services.ai.azure.com/api/projects/{project}/models/{modelName}/versions/{version}?api-version=2025-06-01-preview
Authorization: Bearer {token}
```
...
```bash
azd ai models custom delete --name my-gpt-oss-120B --version 1

# Skip confirmation prompt
azd ai models custom delete --name my-gpt-oss-120B --version 1 --force
```
---
...
| Invalid `weightType` value | All | `UnsupportedWeightType: Only weightType 'FullWeight' is supported in this milestone.` |
| `weightType` not a recognized enum | All | `InvalidWeightType: Value '{value}' is not a valid weightType.` |
...
azd ai models custom deployment-templates \
...
| `weightType` | `string` (enum) | Yes | Yes | `"FullWeight"` for Build. Case-insensitive at ingestion (toLower normalization). Strictly validated — no open strings. Future values: `"LoRA"`, etc. |
