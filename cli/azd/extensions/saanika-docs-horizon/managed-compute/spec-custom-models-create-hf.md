...
> **Note:** The `azd ai models custom create` command accepts `--source` as a local path or remote URL. For public HF models, the HF repo URL can be passed directly. The CLI design spec does not currently define HF-specific flags (`--huggingface-revision`, `--huggingface-token`). See the open item below.
...
- [Part B: Bridge — Resolve Deployment Template](spec-custom-models-deploy.md#part-b-bridge--from-registered-model-to-deployment-template)
- [Part C: Deploy the Custom Model](spec-custom-models-deploy.md#part-c-custom-model-deployment-control-plane)
