...
> **Context:** The current spec ([spec-models-register-lora.md](spec-models-register-lora.md) and [spec-lora-deploy.md](spec-lora-deploy.md)) places adapter attach/detach as **data-plane** operations under `/deployments/{dep}/adapters`. This document evaluates that decision against the alternative of making them **control-plane** (ARM) operations, grounded in the actual platform architecture and capabilities.
