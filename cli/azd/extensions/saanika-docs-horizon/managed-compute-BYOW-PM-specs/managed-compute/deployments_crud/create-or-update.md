# Managed Compute Deployments — Create or Update
...
| `deploymentName` | path | Yes | `string` (1–64 chars, `^[a-zA-Z0-9][a-zA-Z0-9._-]{0,63}$`) | The name for this managed compute deployment. Must be unique within the account. This name is used in the inference URL path. |
...
|---|---|---|---|
| `properties` | [`ManagedComputeDeploymentProperties`](#acceleratordeploymentproperties) | Yes | Properties of the managed compute deployment. |
...
| `acceleratorType` | `string` | Yes | The accelerator type to use for this deployment. Must match one of the entries in the deployment template's `accelerator_maps`. Examples: `H100_80GB`, `H200_141GB`, `A100_80GB`. **Immutable after creation** — changing accelerator type requires deleting and recreating the deployment. |
...
|---|---|---|
| `200 OK` | [`ManagedComputeDeployment`](#response-body) | Update operation completed synchronously (e.g., tag-only update). |
| `201 Created` | [`ManagedComputeDeployment`](#response-body) | Create operation accepted. Deployment provisioning has started. Poll via `Azure-AsyncOperation` header or `GET` on resource URL. |
...
#### 200 OK — Full Resource (Update)

Returned when an update completes synchronously. Includes all read-only properties.
...
#### 201 Created — Minimal Resource (Create)

Returned when a new deployment is created. Contains only the request properties and `provisioningState`. Read-only computed fields (`acceleratorsPerInstance`, `totalAccelerators`, `provisioningDetails`, `routes`, `etag`) are not populated until provisioning progresses.
...
| `type` | `string` | Resource type: `Microsoft.CognitiveServices/accounts/managedComputeDeployments`. |
...
| `properties.routes` | [`InferenceRoutes`](#inferenceroutes) | Inference route paths relative to the account endpoint root, including the full `/managedComputeDeployments/{deploymentName}/` prefix. Populated when `provisioningState` is `Succeeded`. Sourced from the deployment template. |
...
| Update | Fields to set | Method | Behavior |
|---|---|---|---|
| **Scale out/in** | `sku.capacity` | **PATCH** (preferred) or PUT | Changes instance count. Long-running — scale-out provisions new GPUs (10–15 min); scale-in is faster. Use the dedicated [PATCH operation](patch.md) for SKU-only updates. |
| **Change deployment template** | `properties.deploymentTemplate` | PUT | Triggers rolling update of the serving container. May cause brief availability gaps during rollout. |

> Changing `model` or `acceleratorType` is **not supported** — delete and recreate the deployment instead.
...
poller = cog.managed_compute_deployments.begin_create_or_update(
    resource_group_name="my-rg",
    account_name="my-foundry-account",
    deployment_name="gpt-oss-120b-gpu",
    managed_compute_deployment=deployment,
)
...
### `ManagedComputeDeploymentModel` (Custom Models)
...
| `Moving` | Resource is being moved between subscriptions or resource groups. |
| `ResolvingDNS` | DNS records are being provisioned or updated. |
...
|---|---|---|
| `chatCompletionsScoringPath` | `/managedComputeDeployments/{name}/chat/completions` | Chat completions route. Present for OpenAI-compatible models. |
| `messagesApiScoringPath` | `/managedComputeDeployments/{name}/messages` | Messages API route. Present for models supporting the messages API. |
| `swagger` | `/managedComputeDeployments/{name}/swagger.json` | OpenAPI spec for the model's inference API. |

The set of keys depends on the model's runtime. Examples:
- **OpenAI-compatible model** (gpt-oss-120b via vLLM): `{ "chatCompletionsScoringPath": "/managedComputeDeployments/gpt-oss-120b-gpu/chat/completions", "swagger": "/managedComputeDeployments/gpt-oss-120b-gpu/swagger.json" }`
- **Reranker model** (Cohere Rerank): `{ "rerankScoringPath": "/managedComputeDeployments/cohere-rerank/rerank", "swagger": "/managedComputeDeployments/cohere-rerank/swagger.json" }`
- **Embedding model**: `{ "embeddingsScoringPath": "/managedComputeDeployments/embed-v3/embeddings", "swagger": "/managedComputeDeployments/embed-v3/swagger.json" }`
...
| `DeploymentNameConflict` | `409 Conflict` | A serverless deployment with the same name already exists on this account. Deployment names must be unique across both `deployments` and `managedComputeDeployments`. |
